...
@interface RNFSManager()<RNFSDownloaderFinishDelegate>
...
@property (retain) NSMutableArray* downloaderTasks;
...
//                                             selector:@selector(NetWorkChanged) name:@"NetWorkChanged"
//                                               object:nil];
  ...
  params.progressCallback = ^(NSNumber* progress) {
  ...
  if (!self.downloaders) self.downloaders = [[NSMutableDictionary alloc] init];
    ...
    if(_downloaderTasks == nil){
        _downloaderTasks = [[NSMutableArray alloc] init];
    }
  RNFSDownloader* downloader = [RNFSDownloader alloc];
    ...
    [self downloaderTaskAdd:@[downloader,params,[jobId stringValue]]];
//  [_downloaderTasks addObject:@[downloader,params,[jobId stringValue]]];
   
    [self downloaderTaskStart];
...
-(void)RNFSDownloaderFinishCallback:(id)task{
    [self downloaderTaskRemove:task];
}
static int maxTaskCount = 3;
-(void)downloaderTaskRemove:(id)taskDel{
    @synchronized(self) {
        for(id task in _downloaderTasks){
             RNFSDownloader* downloader = task[0];
             
            if([downloader isEqual:taskDel]){
                 [_downloaderTasks removeObject:task];
                [self downloaderTaskStart];
                 break;
             }
         }
...
-(void)downloaderTaskAdd:(id)taskDel{
    @synchronized(self) {
        [_downloaderTasks addObject:taskDel];
    }
}
-(void)downloaderTaskStart{
    @synchronized(self) {
       NSMutableArray* processTasks = [NSMutableArray array];
       int processingTaskCount = 0;
       for(id task in _downloaderTasks){
           if(((RNFSDownloader*)task[0]).taskState == 1){
               processingTaskCount++;
           }else if(((RNFSDownloader*)task[0]).taskState == 0){
               [processTasks addObject:task];
           }
       }
       if(processingTaskCount <= maxTaskCount){
           int n = maxTaskCount-processingTaskCount;
           int processTaskCount = processTasks.count>n ? n :processTasks.count;
           for(int i=0;i<processTaskCount;i++){
               RNFSDownloader* downloader = processTasks[i][0];
               RNFSDownloadParams* params = processTasks[i][1];
               NSString* jobId = processTasks[i][2];
               NSString *uuid = [downloader downloadFile:params];

               [self.downloaders setValue:downloader forKey:jobId];
              if (uuid) {
                  if (!self.uuids) self.uuids = [[NSMutableDictionary alloc] init];
                  [self.uuids setValue:uuid forKey:jobId];
              }
           }
       }
    }
...
//        // 无网络，暂停正在下载任务
//
//        for (int index=0; index<[self.downloaders count]; index ++) {
//            [self.downloaders[index] stopDownload];
//        }
//
//    }else {
//        for (int index=0; index<[self.downloaders count]; index ++) {
//            [self.downloaders[index] resumeDownload];
//        }
//
