...
export default class EnterDatumPoint extends Component {
  ...
  getCurrentPosition = async () => {
    ...
    global.DATUMPOINTVIEW.updateLatitudeAndLongitude({x: map.longitude, y: map.latitude})
  ...
  submit = async () => {
    const point = global.DATUMPOINTVIEW.getLatitudeAndLongitude()
    // 保存选点
      ...
      AsyncStorage.getItem(constants.COLLECT_SCENE_HISTORY_DATASOURCE_ALIAS_KEY)
        .then(async value => {
          ...
          if (value !== null) {
            ...
            const alias = dsAlias//SMap.isAvilableAlias(value)
            if (alias === value) {
