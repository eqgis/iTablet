...
export default class NavigationStartButton extends React.Component {
  ...
  isInBounds = async (position: Point2D, datasetName: string) => {
    const datasources = await SData.getDatasources()
    const datasets = []
    for(let i = 0; i < datasources.length; i++){
      datasets.push(await SData.getDatasetsByDatasource({alias: datasources[i].alias}))
    ...
    for(let i = 0; i < targetDatasource.length; i++) {
      const dataset = targetDatasource[i].filter(dataset => dataset.datasetName === datasetName)[0]
  ...
  realNavigation = async type => {
    try {
      ...
      if (global.CURRENT_NAV_MODE === 'INDOOR') {
        ...
        const naviData = this.props.getNavigationDatas()
        //判断是否在当前导航的室外数据集范围内
        const isInBounds = await this.isInBounds(
  ...
  renderItem = ({ item }) => {
    ...
    let str = ''
    const thenInfo = this.language === 'CN' ? '然后' : 'and then'
    ...
    const icon = this.getIconByType(turnType)
    return (
      <View>
        <View
          style={{
