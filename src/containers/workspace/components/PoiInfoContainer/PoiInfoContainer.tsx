...
export default class PoiInfoContainer extends React.Component {
  ...
  componentDidMount() {
    const animParams = this.getAnimParams()
  ...
  setVisible = (visible, state = {
    ...
    const value = visible ? 0 : scaleSize(-screen.getScreenHeight())
    Animated.timing(this.bottom, {
      toValue: value,
      duration: 400,
      useNativeDriver: false,
  ...
  getSearchResult = async (params, cb) => {
    try {
      ...
      LocateUtils.getSearchResult(params, { x: currentLocation.longitude, y: currentLocation.latitude }, data => {
        if (data) {
          const newState = {resultList: data.resultList, radius: 50000, showList: true}
  ...
  onListItemClick = async item => {
    ...
    if (searchClickedInfo.isClicked) {
      ...
      const tempName = global.PoiTopSearchBar.state.defaultValue
      global.PoiTopSearchBar.setState({ defaultValue: item.pointName })
      this.showTable()
      setTimeout(async () => {
        const tempList = JSON.parse(JSON.stringify(this.state.resultList))
        this.tempResult = {
          name: tempName,
          tempList,
        }
  ...
  clear = async () => {
    const rel1 = await SMap._removePOICallout()
    const rel2 = await SMap._removeAllCallout()
    return rel1 && rel2
  }
  ...
  navitoHere = async () => {
    ...
    global.STARTX = position.longitude
    global.STARTY = position.latitude
