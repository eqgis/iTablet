...
import { AppLog, AppPath, AppToolBar, CheckSpell, DataHandler, Toast } from "@/utils"
...
async function _downloadExamples(type: ExternalDataType, examples: ExampleData[]) {
  for(let i = 0; i < examples.length; i++) {
    ...
    if(AppProgress.isInProgress(downloadKey)) {
      Toast.show(getLanguage().Data.DOWNLOADING)
      return
    }
  }
...
async function _downloadExample(type: ExternalDataType, exampleData: ExampleData) {
  ...
  let result: boolean
  //尝试读取以前下载的示范数据
  const items = await DataHandler.getExternalData(homePath + AppPath.Common.path + '/' + exampleData.dir, [type])
  if(items.length === 0) {
    Toast.show(getLanguage().Data.DOWNLOADING)
    ...
    if(result) {
      Toast.show(getLanguage().Data.IMPORTING)
      const items = await DataHandler.getExternalData(homePath + AppPath.Common.path + '/' + exampleData.dir, [type])
      if(items.length > 0) {
        //TODO 导入所有数据
        result = await DataHandler.importExternalData(items[0])
        result && AppToolBar.resetTabData()
        Toast.show(result ? getLanguage().Data.IMPORT_SUCCESS : getLanguage().Data.IMPORT_FAIL)
      } else {
        AppLog.error('没有数据')
      }
    } else {
      Toast.show(getLanguage().Data.DOWNLOAD_FAIL)
    }
  } else {
