...
async function action(type) {
  ...
  let selectedDatas = _params.getNavigationDatas()
  let {selectedDatasets, selectedDatasources,currentDatasource,currentDataset} = selectedDatas
  let mapDatasource = data[0]
  let mapDataset = data[1]
  mapDatasource.data.map(item => {
    selectedDatasources && selectedDatasources.map(ds => {
  ...
  mapDataset.data.map(item => {
    selectedDatasets && selectedDatasets.map(dt => {
