...
async function shareMap(type, list = [], name = '') {
  try {
    ...
    if (ToolbarModule.getData().isSharing) {
      Toast.show(getLanguage(GLOBAL.language).Prompt.SHARING, {duration:1000})
      // ConstInfo.SHARE_WAIT)
      return
    }
    ToolbarModule.getParams().setToolbarVisible &&
      ToolbarModule.getParams().setToolbarVisible(false)
    Toast.show(getLanguage(GLOBAL.language).Prompt.SHARE_PREPARE, {duration:1000})
      ...
      ToolbarModule.getParams().exportWorkspace(
        ...
        async (result, path) => {
          if (!result) {
            ...
            Toast.show(ConstInfo.EXPORT_WORKSPACE_FAILED, {duration:1000})
            return
          }
          // 分享
          ...
          Toast.show(getLanguage(GLOBAL.language).Prompt.SHARE_START, {duration:1000})
...
function showSaveDialog(type) {
  if (
    ...
    Toast.show(getLanguage(GLOBAL.language).Prompt.PLEASE_LOGIN_AND_SHARE, {duration:1000})
    // '请登陆后再分享')
    return
  }
  if (!ToolbarModule.getParams().map.currentMap.name) {
    Toast.show(ConstInfo.PLEASE_SAVE_MAP, {duration:1000})
    return
  }
