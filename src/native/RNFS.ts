...
type DownloadFileOptions = {
  ...
  headers?: HttpHeaders;        // An object of headers to be passed to the server
...
type DownloadBeginCallbackResult = {
  ...
  headers: HttpHeaders;       // The HTTP response headers from the server
};
...
function readFileGeneric(filepath: string, encodingOrOptions: string | null, command: Function) {
  ...
  return command(normalizeFilePath(filepath)).then((b64: any) => {
    var contents;
...
var RNFS = {
  ...
  stat(filepath: string): Promise<StatResult> {
    return RNFSManager.stat(normalizeFilePath(filepath)).then((result: { ctime: number; mtime: number; size: any; mode: any; originalFilepath: any; type: any; }) => {
      return {
        'path': filepath,
  ...
  read(filepath: string, length: number = 0, position: number = 0, encodingOrOptions?: any): Promise<string> {
    ...
    return RNFSManager.read(normalizeFilePath(filepath), length, position).then((b64: any) => {
      var contents;
  ...
  uploadFiles(options: UploadFileOptions): { jobId: number, promise: Promise<UploadResult> } {
    ...
    return {
      jobId,
      promise: RNFSManager.uploadFiles(bridgeOptions).then((res: any) => {
        subscriptions.forEach(sub => sub.remove());
        return res;
      })
    };
